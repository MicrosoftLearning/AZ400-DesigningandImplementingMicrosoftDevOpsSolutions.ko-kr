...
# Azure Load Testing을 사용하여 애플리케이션 성능 모니터링
...
2. 새 파이프라인 **(또는 처음 만드는 파이프라인인 경우 파이프라인 만들기)을 클릭합니다**.
...
```
8. YAML 정의 끝부분의 새 줄에 커서를 설정합니다. **커서를 이전 작업 수준의** 들여쓰기 위치에 배치해야 합니다.

    > **참고**: 이 커서 위치에 새 작업이 추가됩니다.

9. 포털의 오른쪽에서 [도우미** 표시]를 클릭합니다**. 작업 목록에서 Azure 앱 서비스 배포** 작업을 검색하고 선택합니다**.
10. **Azure App Service 배포** 창에서 다음 설정을 지정하고

    - **Azure 구독** 드롭다운 목록에서 랩의 앞부분에서 Azure 리소스를 배포한 Azure 구독을 선택합니다. 필요한 경우(만든 첫 번째 파이프라인인 경우에만) 권한 부여**를 클릭하고 **메시지가 표시되면 Azure 리소스 배포 중에 사용한 것과 동일한 사용자 계정을 사용하여 인증합니다.
    - Windows에서 웹앱에 대한 App Service 유형** 지점의 유효성을 검사**합니다.
    - **App Service 이름** 드롭다운 목록에서 랩에서 이전에 배포한 웹앱의 이름을 선택합니다(**az400eshoponweb...).
    - **패키지 또는 폴더** 텍스트 상자에서 기본값을 `$(Build.ArtifactStagingDirectory)/**/Web.zip`으로 **업데이트**합니다.
11. **추가** 단추를 클릭하여 도우미 창에서 설정을 확인합니다.

    > **참고**: 그러면 YAML 파이프라인 정의에 배포 작업이 자동으로 추가됩니다.

12. 편집기에 추가된 코드 조각은 azureSubscription 및 WebappName 매개 변수의 이름을 반영하여 아래와 유사해야 합니다.
...
> **참고**: 이 랩에서는 **packageForLinux** 매개 변수를 사용하면 안 되지만 Windows나 Linux에서는 해당 매개 변수를 사용할 수 있습니다.
    ...
    ```
13. yml-file에 업데이트를 저장하기 전에 보다 명확한 이름을 지정합니다. yaml 편집기 창 위에 EShopOnweb/azure-pipelines-#.yml** 표시됩니다**. (여기서 #은 숫자이며 일반적으로 1이지만 설정에서 다를 수 있습니다.) 해당 파일 이름을** 선택하고 **이름을 **m09l16-pipeline.yml**

14. **저장**을 클릭하고 **저장** 창에서 **저장**을 다시 클릭하여 변경 내용을 마스터 분기에 직접 커밋합니다.

    > **참고**: 원래 CI-YAML이 새 빌드를 자동으로 트리거하도록 구성되지 않았기 때문에 이 빌드를 수동으로 시작해야 합니다.

15. Azure DevOps 왼쪽 메뉴에서 **파이프라인**으로 이동한 후 **파이프라인을** 다시 선택합니다. 다음으로, [모두 **]를 선택하여 **최근 파이프라인 정의뿐만 아니라 모든 파이프라인 정의를 엽니다.

(참고: 이전 랩 연습에서 이전 파이프라인을 모두 유지한 경우 이 새 파이프라인이 기본값 ** 을 재사용했을 수 있습니다.아래 스크린샷과 같이 파이프라인의 EShopOnWeb(#)** 시퀀스 이름입니다. 파이프라인을 선택합니다(대부분 시퀀스 번호가 가장 높은 파이프라인, 편집을 선택하고 m09l16-pipeline.yml 코드 파일을 가리키는지 확인) 

![](images/m3/eshoponweb-m9l16-pipeline.png)

11. 표시되는 창에서 실행을** 클릭하여 **이 파이프라인을 실행하도록 확인하고 실행을 한 번 더 클릭하여 **** 확인합니다.
12. 서로 다른 2가지 스테이지인 **.NET Core 솔루션 빌드** 및 **Azure 웹앱에 배포**가 표시됩니다.
13. 파이프라인이 시작될 때까지 기다립니다. 

16. **빌드 단계 중에 표시되는 경고를 무시** 합니다. 빌드 단계가 성공적으로 완료될 때까지 기다립니다. (실제 빌드 단계를 선택하여 로그에서 자세한 내용을 볼 수 있습니다.)

17. 배포 단계가 시작되고 나면 필요한** 사용 권한과 **다음을 나타내는 주황색 막대가 표시됩니다.
...
18. **보기**를 클릭합니다.
19. **검토 대기** 창에서 **허용**을 클릭합니다.
20. **허용 팝업** 창에서 메시지의 유효성을 검사하고 **허용**을 클릭하여 확인합니다.
21. 이렇게 하면 배포 스테이지가 시작됩니다. 이 스테이지가 성공적으로 완료될 때까지 기다립니다.
...
1. EShopOnWebLoadTesting** Azure Load Testing 리소스 블레이드 내에서 **테스트**로 이동합니다**. **+만들기** 메뉴 옵션을 클릭하고 URL 기반 테스트** 만들기를 선택합니다**. 
2. 다음 매개 변수 및 설정을 완료하여 부하 테스트를 만듭니다.
- **테스트 URL**: 이전 연습에서 배포한 Azure 앱 서비스의 URL을 입력합니다(az400eshoponweb... azurewebsites.net)**https:// 포함**
- **부하** 지정: 가상 사용자
- **가상 사용자** 수: 50
- **테스트 기간(분)**: 5
- **램프업 시간(분)**: 1
3. 검토 및 만들기**를 클릭하여 테스트 구성을 **확인합니다(다른 탭에서는 변경하지 마세요). 만들기**를 한 번 더 클릭합니다**.
4. 그러면 부하 테스트가 시작됩니다. 그러면 테스트가 5분 동안 실행됩니다. 
5. 테스트가 실행되면 EShopOnWebLoadTesting** Azure Load Testing 리소스 페이지로 다시 **이동하고 테스트로 이동하여 **테스트를**** 선택하고 **테스트 **Get_eshoponweb...**
6. 위쪽 메뉴에서 만들기 **, **URL 기반 테스트** 만들기를 클릭하여 **두 번째 부하 테스트를 만듭니다.
7. 다음 매개 변수 및 설정을 완료하여 다른 부하 테스트를 만듭니다.
...
- **테스트 기간(분)**: 5
- **램프업 시간(분)**: 1
8. 검토 + 만들기를 클릭하고 다시 한 번 만들기****** 를 클릭하여 테스트 구성을 **확인합니다.
9. 테스트는 약 5분 동안 실행됩니다.
...
1. Azure Load Testing**에서 **테스트**로 이동합니다**. 테스트 정의 중 하나를 선택하여 테스트 중 하나를 클릭하여** 더 자세한 보기를 **엽니다. 그러면 더 자세한 테스트 페이지로 리디렉션됩니다. 여기에서 결과 목록에서 TestRun_mm/dd/yy-hh:hh**를 선택하여 **실제 실행의 세부 정보를 확인할 수 있습니다.
2. 자세한 **TestRun** 페이지에서 Azure Load Testing 시뮬레이션의 실제 결과를 식별합니다. 일부 값은 다음과 같습니다.
- 로드/총 요청 수
- 기간
- 응답 시간(90번째 백분위수 응답 시간을 반영하는 시간(초)의 결과를 보여 줍니다. 즉, 요청의 90%에 대해 응답 시간이 지정된 결과 내에 있음을 의미합니다.
- 초당 요청의 처리량
3. 아래에서는 이러한 값 중 몇 가지가 대시보드 그래프 선 및 차트 보기를 사용하여 표시됩니다.
4. 시뮬레이션된 두 테스트의 결과를** 서로 비교하고 **더 많은 사용자가 App Service 성능에 미치는 영향을** 파악하는 데 몇 분 **정도 걸립니다.
...
8. jmeter를** 폴더 이름으로 지정**하고 **파일 이름에 대한 placeholder.txt** 지정합니다(참고: 폴더를 빈 폴더로 만들 수 없음).
