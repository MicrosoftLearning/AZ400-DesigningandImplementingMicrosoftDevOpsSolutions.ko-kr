...
2. 에이전트 풀을 삭제합니다.
3. PAT 토큰을 해지합니다.
4. Repos/.ado/eshoponweb-ci-pr.yml에서 이동하여 편집을 선택하고 **13-15줄(에이전트 풀 코드 조각)을 제거하고 원래대로 다시 `vmImage: windows-latest` 변경하여 eshoponweb-ci-pr.yml** 파일의 변경 내용을 **되돌**립니다. (이는 향후 랩 연습에서 동일한 샘플 파이프라인 파일을 사용하기 때문입니다.) 
