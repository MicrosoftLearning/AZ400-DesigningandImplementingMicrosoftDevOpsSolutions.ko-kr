...
1. **Azure App Service 배포** 창에서 다음 설정을 지정하고
   ...
   - **애플리케이션 및 구성 설정** 추가`-UseOnlyInMemoryDatabase true -ASPNETCORE_ENVIRONMENT Development`
...
1. 배포 단계가 시작되고 나면 필요한** 사용 권한과 **다음을 나타내는 주황색 막대가 표시됩니다.
...
1. 배포된 사이트가 eShopOnWeb 전자 상거래 웹 사이트를 표시하는 새 브라우저 탭에서 예상대로 로드되는지 확인합니다.
...
1. Azure DevOps 프로젝트 **eShopOnWeb_MultiStageYAML** 파이프라인**으로 이동합니다**.
