...
1. **파이프라인>파일로** 이동합니다.
1. **파이프라인 만들기 단추(파이프라인**이 없는 경우) 또는 **새 파이프라인 단추(이미 생성된 파이프라인**이 있는 경우)를 클릭합니다.
...
1. **설정** 섹션에서 ID**를 클릭한 **다음 시스템 할당** 섹션에서 상태 켜**** 짐으로 **전환하고 > 저장**을 클릭하고 **작업이 완료되기까지 몇 초 정도 기다립니다.
1. App Configuration 서비스로 돌아간 후 **액세스 제어**를 클릭한 다음 **역할 할당 추가**를 클릭합니다.
1. **역할** 섹션에서 App Configuration 데이터 판독기를** 선택합니다**.
