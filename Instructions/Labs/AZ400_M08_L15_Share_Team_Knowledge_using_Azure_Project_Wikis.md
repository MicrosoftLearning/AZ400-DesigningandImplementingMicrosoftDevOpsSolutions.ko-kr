...
1. **파일** 창의 왼쪽에 있는 리포지토리 폴더 및 파일 계층 구조 목록에서 **src** 폴더를 확장하고 **웹 -> wwwroot -> 이미지** 하위 폴더로 이동합니다. **이미지** 하위 폴더에서 **brand.png** 항목을 찾고 마우스 포인터를 오른 끝으로 가져갑니다. **자세히** 메뉴를 나타내는 세로 줄임표(점 3개) 기호를 표시되면 **다운로드**를 클릭하여 랩 컴퓨터의 로컬 **다운로드** 폴더에 **brand.png** 파일을 다운로드합니다.
