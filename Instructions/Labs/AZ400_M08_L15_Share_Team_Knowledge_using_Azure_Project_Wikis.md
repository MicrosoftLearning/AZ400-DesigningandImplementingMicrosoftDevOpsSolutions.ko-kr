...
1. **파일** 창의 왼쪽에 있는 리포지토리 폴더 및 파일 계층 구조 목록에서 **src** 폴더를 확장하고 **Web -> wwwroot -> images** 하위 폴더로 이동합니다. **이미지** 하위 폴더에서 **brand.png** 항목을 찾고 마우스 포인터를 오른 끝으로 가져갑니다. **자세히** 메뉴를 나타내는 세로 줄임표(점 3개) 기호를 표시되면 **다운로드**를 클릭하여 랩 컴퓨터의 로컬 **다운로드** 폴더에 **brand.png** 파일을 다운로드합니다.

    > **참고**: 이 이미지는 이 랩의 다음 연습에서 사용합니다.

1. Wiki 원본 파일을 Repos 현재 폴더 구조 내의 별도 폴더에 저장합니다. **Repos** 내에서 **파일**을 선택합니다. 폴더 구조체 위에 **eShopOnWeb** Repo 제목이 있습니다. **줄임표(점 3개)를 선택**하고, **새로 만들기 / 폴더**를 선택하고, **`Documents`** 를 새 폴더 이름의 제목으로 제공합니다. 리포지토리에서는 빈 폴더를 만들 수 없으므로 **`READ.ME`** 를 새 파일 이름으로 제공합니다.
...
1. Wiki 페이지 **제목** 필드에 "`Welcome to our Online Retail Store!`"를 입력합니다.
...
1. **페이지 제목** 텍스트 상자에 **`Project Design`** 을 입력합니다.
...
1. 새로 추가된 **##** 문자 바로 뒤에 **`Azure DevOps OAuth 2.0 Authorization Flow`** 을 입력하고 **Enter** 키를 누릅니다.
...
1. **페이지 저장** 대화상자에서 **`Authentication and authorization section with the OAuth 2.0 Mermaid diagram`** 를 입력하고 **저장**을 클릭합니다.
...
1. 새로 추가된 **##** 문자 바로 뒤에 **`User Interface`** 을 입력하고 **Enter** 키를 누릅니다.
...
1. 편집기 창 오른쪽 위에서 **저장** 단추 옆의 아래쪽 캐럿을 클릭하고 드롭다운 메뉴에서 **수정 메시지와 함께 저장**을 선택합니다.
1. **페이지 저장** 대화 상자에서 **`User Interface section with the eShopOnWeb image`** 를 입력하고 **저장**을 클릭합니다.
...
1. 랩 컴퓨터에서 **eShopOnWeb** 프로젝트의 **Wiki 창**을 표시하는 Azure DevOps 포털에서 **프로젝트 디자인** Wiki의 콘텐츠를 선택한 상태로 오른쪽 상단 모서리의 세로 줄임표 기호를 클릭하고 드롭다운 메뉴에서 **수정 버전 보기**를 클릭합니다.
...
1. 페이지 편집기 창의 **페이지 제목** 텍스트 상자에 **`Project Design Overview`** 입력 후 **저장**을 클릭한 다음 **닫기**를 클릭합니다.
