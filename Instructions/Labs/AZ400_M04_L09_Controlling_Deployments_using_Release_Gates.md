...
이 작업에서는 카나리아** 및 **프로덕션** 환경을 나타내는 **두 개의 Azure 웹앱을 만들어 Azure Pipelines를 통해 애플리케이션을 배포합니다.
...
1.  고유한 앱 이름을 사용하여 두 개의 웹앱을 만듭니다.
    ...
    > **참고**: 카나리아 웹앱의 이름을 기록합니다. 이 랩의 후반부에서 필요합니다.
...
1. 리소스 목록에서 카나리아** 웹앱을 클릭합니다**.
1. **카나리아** 웹앱 페이지의 왼쪽 **세로 메뉴에서 설정** 섹션에서 Application Insights**를 클릭합니다**.
...
1. 파이프라인 탭에서 **아티팩트** 추가 사각형을 선택하고 **원본(빌드 파이프라인)** 필드에서 eShopOnWeb**** 을 선택합니다**.** **추가**를 클릭하여 아티팩트 선택 사항을 확인합니다.
1. **아티팩트** 사각형에서 **연속 통합 트리거**(번개 모양)가 표시되어 있는지 살펴보십시오. **연속 배포 트리거** 설정을 열려면 이를 클릭합니다. 연속 배포 트리거를 클릭하면 전환을 토글하여 이를 활성화할 수 있습니다. 다른 모든 설정은 기본적으로 그대로 두고 오른쪽 위 모서리에 있는 x 표시를 **클릭하여 연속 배포 트리거** 창을 닫습니다**.**
1. **카나리아 환경** 단계 내에서 1개의 작업, 2개의 작업** 레이블을 클릭하고 **이 단계 내의 작업을 검토합니다.

    > **참고**: Canary 환경에는 각각 아티팩트 패키지를 Azure 웹앱에 게시하는 1개의 작업이 있습니다.

1. **모든 파이프라인 > 새 릴리스 파이프라인** 창에서 **Canary** 스테이지가 선택되었는지 확인합니다. **Azure 구독** 드롭다운 목록에서 Azure 구독을 선택하고 권한 부여**를 클릭합니다**. 메시지가 표시되면 Azure 구독에서 소유자 역할이 있는 사용자 계정을 사용하여 인증합니다.
...
1. 파이프라인 창에서 **eShopOnWeb** 빌드 파이프라인을 **나타내는 항목을 클릭한 다음 eShopOnWeb** 창에서 ****파이프라인** 실행을** 클릭합니다.
...
1. 세로 탐색 창**의 파이프라인** 섹션에서 릴리스**를 클릭하고 **eShopOnWeb** 창에서 **가장 최근 릴리스를 나타내는 항목을 클릭합니다.
1. **eShopOnWeb > 릴리스-1** 블레이드에서 릴리스 진행률을 추적하고 두 웹앱에 대한 배포가 성공적으로 완료되었는지 확인합니다.
...
이 작업에서는 카나리아 환경에 대한 배포 후 게이트를 사용하도록 설정합니다.
...
이 작업에서는 먼저 카나리아 웹앱에 대한 몇 가지 경고를 생성한 다음 릴리스 게이트를 사용하도록 설정된 릴리스 프로세스를 추적합니다.
...
1. Azure Portal의 "리소스, 서비스 및 문서 검색" 필드에 Application Insights를 입력**하고 이전 연습에서 만든 Canary-AppInsights** 리소스를 선택합니다**.** 다음으로 경고**로 이동합니다**. 
1. 결과 목록에 1**개 이상의 **새 경고가 있어야 하며 **, 심각도 2**가 경고를** 입력**하여 Azure Monitor의 경고 서비스를 엽니다.
1. 심각도 2 - 경고**가 목록에 표시되는 1**개 이상의 **Failed_Alert**** 이 있어야 합니다. 이전 연습에서 존재하지 않는 웹 사이트 URL 주소의 유효성을 검사할 때 트리거가 발생했습니다.

    > ** 참고:** 경고가 아직 표시되지 않은 경우 몇 분 정도 더 기다리십시오. 
...
이 작업에서는 Azure Cloud Shell을 사용하여 이 랩에서 Azure 리소스 프로비저닝을 제거하여 불필요한 요금을 제거합니다.
