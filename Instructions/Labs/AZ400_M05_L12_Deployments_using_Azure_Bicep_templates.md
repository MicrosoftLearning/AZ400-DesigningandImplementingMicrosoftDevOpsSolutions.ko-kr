...
1. 줄 20에서 `publicIPAllocationMethod` 매개 변수의 기본값을 `Dynamic`에서 `Static`으로 변경합니다.

1. 줄 27에서 `publicIpSku` 매개 변수의 기본값을 `Basic`에서 `Standard`로 변경합니다.
...
#### 작업 3: 템플릿 모듈을 사용하도록 simple-windows-vm 템플릿 수정

이 작업에서는 이전 작업에서 만든 템플릿 모듈을 참조하도록 `simple-windows-vm.bicep` 템플릿을 수정합니다.
...
1. 배포가 완료될 때까지 기다린 후 결과를 검토합니다.
   ...
   > **참고**: 파이프라인에 이전에 만든 서비스 연결을 사용할 수 있는 권한을 부여해야 합니다.
