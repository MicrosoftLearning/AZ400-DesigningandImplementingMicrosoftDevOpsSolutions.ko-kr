...
- Git 2.47.0 이상을 아직 설치하지 않았다면 웹 브라우저를 시작하고 [Windows용 Git 다운로드 페이지](https://gitforwindows.org/)로 이동하여 Git를 다운로드한 다음 설치합니다.
...
- **Git:** Git은 분산 버전 제어 시스템입니다. 개발자 머신 등의 로컬 환경에 Git 리포지토리를 저장할 수 있습니다. 각 개발자는 자신의 개발 컴퓨터에 소스 리포지토리의 복사본을 가지고 있습니다. 개발자는 개발 컴퓨터에서 각 변경 세트를 커밋하고 네트워크 연결 없이 기록 및 비교와 같은 버전 제어 작업을 수행할 수 있습니다.
...
#### 작업 1: Git 및 Visual Studio Code 구성
...
1. 현재 터미널에서 **PowerShell**을 실행 중인지 확인합니다. **터미널** 창 오른쪽 위의 드롭다운 목록에 **1: powershell**이 표시되어 있는지 확인하면 됩니다.
...
#### 작업 2: (완료된 경우 건너뛰기) 팀 프로젝트 만들기 및 구성
...
1. 랩 컴퓨터의 브라우저 창에서 Azure DevOps 조직을 엽니다. **새 프로젝트**를 클릭합니다. 프로젝트 이름을 **eShopOnWeb**으로 설정하고 **작업 항목 프로세스** 드롭다운에서 **스크럼**을 선택합니다. **만들기**를 클릭합니다.
    ...
    ![새 프로젝트 패널 만들기의 스크린샷.](images/create-project.png)

#### 작업 3: (완료된 경우 건너뛰기) eShopOnWeb Git 리포지토리 가져오기
...
#### 작업 4: (완료된 경우 건너뛰기) main 분기를 default 분기로 설정
...
이 연습에서는 Visual Studio Code를 사용하여 **eShopOnWeb** 리포지토리의 **main** 분기에 변경 내용을 커밋합니다.

> **참고**: **main** 분기는 **eShopOnWeb** 리포지토리의 default 분기로서 랩의 나머지 부분에 사용할 분기입니다.
...
1. 명령 팔레트 프롬프트에서 **Git: Clone** 명령을 실행합니다.
    ...
    ![VS Code 명령 팔레트의 스크린샷.](images/vscode-command.png)
...
1. **폴더 선택** 대화 상자 내에서 C: 드라이브로 이동하여 이름이 **Git**인 새 폴더를 만들고 선택한 다음 **리포지토리 대상으로 선택**을 클릭합니다.
1. 메시지가 표시되면 Azure DevOps 계정에 로그인합니다.
1. 복제 프로세스가 완료된 후 메시지가 표시되면 Visual Studio Code에서 **열기**를 클릭하여 복제한 리포지토리를 엽니다.
...
1. **소스 제어** 탭을 선택한 상태로 창 위쪽의 텍스트 상자에 커밋 메시지로 **`My commit`** 을 입력하고 **Ctrl+Enter**를 눌러 로컬에서 변경 내용을 커밋합니다.
    ...
    ![첫 번째 커밋의 스크린샷.](images/first-commit.png)

1. 변경 내용을 자동으로 스테이징하여 바로 커밋할 것인지를 묻는 메시지가 표시되면 **항상**을 클릭합니다.

    > **참고**: **스테이징** 과정은 이 랩 뒷부분에서 설명합니다.
...
1. 목록 맨 위에 커밋이 표시되는지 확인합니다.
    ...
    ![ADO 리포지토리 커밋의 스크린샷.](images/ado-commit.png)
...
1. **소스 제어** 탭을 선택한 상태로 창 위쪽의 텍스트 상자에 커밋 메시지로 **`Added comments`** 을 입력합니다.

    ![스테이징된 변경 내용의 스크린샷.](images/staged-changes.png)
...
Git에서는 끌어오기 요청을 통해 **분기 및 병합** 기능을 사용합니다. 따라서 개발 과정의 커밋 기록이 항상 시간순으로 제공되는 것은 아닙니다. 그러므로 기록을 사용해 버전을 비교할 때는 두 시점 간의 파일 변경 내용이 아니라 두 커밋 간의 파일 변경 내용을 확인해야 합니다. 가령 2주 전에 기능 분기에서 변경 내용이 커밋되었는데 이 분기가 어제 기본 분기에 병합되어 기본 분기의 파일에서는 해당 변경 내용이 최신 항목일 수도 있습니다.
...
1. Visual Studio Code 창에서 **소스 제어** 탭을 연 상태로 파일의 스테이징되지 않은 버전을 나타내는 **Constants.cs**를 선택합니다.
    ...
    ![파일 비교의 스크린샷.](images/file-comparison.png)
...
1. 줄임표를 클릭하고 드롭다운 메뉴에서 **파일 찾아보기**를 선택한 후 결과를 검토합니다.

    ![커밋 찾아보기의 스크린샷.](images/commit-browse.png)
...
1. 팝업 창에서 **+ 새 분기 만들기...** 를 선택합니다.
    ...
    ![분기 만들기 옵션의 스크린샷.](images/create-branch.png)

1. **분기를 만들 참조 선택** 텍스트 상자에서 **main**을 참조 분기로 선택합니다.
...
1. 줄임표를 클릭하고 팝업 메뉴에서 분기 삭제를 선택한 다음 **분기 삭제**를 확인하라는 메시지가 표시되면 **삭제**를 클릭합니다.
    ...
    ![분기 삭제 옵션의 스크린샷.](images/delete-branch.png)
...
1. **분기**의 **내 항목** 탭이 표시되는 웹 브라우저로 이동
...
1. 팝업 메뉴에서 줄임표를 클릭하고 **분기 복원**을 선택합니다.
    ...
    ![분기 복원 옵션의 스크린샷.](images/restore-branch.png)
...
1. 줄임표를 클릭하고 팝업 메뉴에서 **분기 정책**을 선택합니다.
    ...
    ![분기 정책 옵션의 스크린샷.](images/branch-policies.png)
...
1. 리포지토리 설정의 **main** 탭에서 **연결된 작업 항목 확인** 옵션을 사용하도록 설정하고 **필수** 옵션으로 그대로 둡니다.

    ![정책 설정의 스크린샷.](images/policy-settings.png)
...
1. **커밋 > 커밋**을 클릭합니다. 경고가 표시됩니다. 기본 분기를 변경하는 건 끌어오기 요청을 사용하는 경우에만 가능합니다.
    ...
    ![정책 거부 커밋의 스크린샷.](images/policy-denied.png)
...
1. 끌어오기 요청을 만들도록 제안하는 메시지가 팝업 창에 표시됩니다. 이제 **dev** 분기가 **main**과 비교하여 변경 내용에서 앞서 있기 때문입니다. **끌어오기 요청 만들기**를 클릭합니다.
    ...
    ![끌어오기 요청 만들기 옵션의 스크린샷.](images/create-pr.png)
...
1. 오른쪽 옵션에서 **작업 항목** 옆의 **+** 단추를 클릭합니다. 이전에 만든 작업 항목을 클릭하여 끌어오기 요청에 연결합니다. 요구 사항의 변경 상태 중 하나가 표시됩니다.
    ...
    ![링크 작업 항목의 스크린샷.](images/link-wit.png)
...
1. **태그 만들기** 패널의 **이름** 텍스트 상자에 **`v1.1.0-beta`** 를 입력하고 **기준** 드롭다운 목록에서는 **main** 항목을 선택된 상태로 둡니다. **설명** 텍스트 상자에는 **`Beta release v1.1.0`** 을 입력하고 **만들기**를 클릭합니다.
...
제시된 순서대로 다른 강좌 랩을 실행하는 경우 이 랩에 구성된 분기 정책은 향후 랩에서의 연습을 방해합니다. 따라서 구성된 분기 정책을 제거하는 것이 좋습니다.

1. Azure DevOps **eShopOnWeb** 프로젝트 보기에서 **Repos**로 이동하여 **분기**를 선택합니다. **분기** 창의 **내 항목** 탭을 선택합니다.
1. **분기** 창의 **내 항목** 탭에서 **기본** 분기 항목 위에 마우스 포인터를 올리면 오른쪽에 줄임표 기호(...)가 표시됩니다.
1. 줄임표를 클릭하고 팝업 메뉴에서 **분기 정책**을 선택합니다.

    ![정책 설정의 스크린샷.](images/policy-settings.png)

1. 리포지토리 설정의 **기본** 탭에서 **최소 검토자 수 필요** 옵션을 사용하지 않도록 설정합니다.
1. 리포지토리 설정의 **기본** 탭에서 **링크된 작업 항목 확인** 옵션을 사용하지 않도록 설정합니다.

    ![분기 정책의 스크린샷.](images/branch-policies.png)

1. 이제 기본 분기에 대한 분기 정책을 사용하지 않도록 설정/제거했습니다.

## 검토

이 랩에서는 Azure Repos에서 버전 제어에 Git을 사용하는 방법을 알아보았습니다.
